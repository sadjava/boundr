...
уникальна по `(video_id, pipeline)`, поэтому сырые результаты двух режимов сохраняются
раздельно.

**Но сравнить их в интерфейсе нельзя.** `annotations` уникальна по `video_id`, колонки
`pipeline` там нет, и `upsert_working_annotation` перезаписывает `annotations.data` при
каждом прогоне — вместе с правками пользователя. Второй режим, запущенный на том же видео,
затирает рабочую разметку первого.

Код обоих режимов лежит в одном пакете `ml-service/app/inference/pegasus/`. Различаются
режимы телом `response_format`, значением `analysis_mode` и разбором ответа; загрузка
файла, поллинг, таймаут, построение промпта и приведение к контракту у них общие.
...
То же происходит в БД с `annotations.data` — таблица уникальна по `video_id`, так что
второй прогон затирает рабочую разметку и правки пользователя. Раздельно по пайплайнам
хранятся только строки `inferences`.
